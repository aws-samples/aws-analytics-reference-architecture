...
test('Predefined source Amazon S3 Bucket', () => {
  ...
  expect(predefinedGenerator.dataset.location.bucketName).toEqual(Dataset.RETAIL_100GB_STORE_SALE.location.bucketName);
});

test('Predefined source Amazon S3 prefix', () => {
  // Test if the Amazon S3 Prefix for the source is correct
  expect(predefinedGenerator.dataset.location.objectKey).toEqual(Dataset.RETAIL_100GB_STORE_SALE.location.objectKey);
});